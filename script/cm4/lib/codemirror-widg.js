...
(function(mod) {
  ...
  // New event added by matching https://github.com/MDeSchaepmeester/CodeMirror

  // Run the given mode's parser over a line, calling f for each token.
  function runMode(cm, lineNo, text, mode, state, f, forceToEnd) { // -EB added lineNo
    var flattenSpans = mode.flattenSpans;
    if (flattenSpans == null) flattenSpans = cm.options.flattenSpans;
    var curStart = 0, curStyle = null;
    var stream = new StringStream(text, cm.options.tabSize), style;
    if (text == "" && mode.blankLine) mode.blankLine(state);
    var tokenStart = {line: lineNo, ch: 0}, tokenEnd = {line: lineNo, ch: 0}; // - EB
    while (!stream.eol()) {
      tokenStart.ch = stream.start; // - EB
      ...
      var substr = stream.current(); // - EB
      if (!flattenSpans || curStyle != style) {
        tokenEnd.ch = stream.pos;
        if (style !== null) signal(cm, "tokenParsed", cm, tokenStart, tokenEnd, style, substr); // - EB
    ...
    // start - EB
    if(style !== null && flattenSpans) {
      tokenEnd.ch = stream.pos;
      signal(cm, "tokenParsed", cm, tokenStart, tokenEnd, style, substr);
    }
    // end - EB
  ...
  function highlightLine(cm, line, state, forceToEnd) {
    ...
    runMode(cm, lineNo(line), line.text, cm.doc.mode, state, function(end, style) { // - EB
      st.push(end, style);
    }, forceToEnd);
    ...
    for (var o = 0; o < cm.state.overlays.length; ++o) {
      ...
      runMode(cm, lineNo(line), line.text, overlay.mode, true, function(end, style) { // - EB
        var start = i;
